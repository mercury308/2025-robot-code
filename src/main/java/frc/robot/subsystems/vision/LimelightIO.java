...
public class LimelightIO implements VisionIO {
	...
	public LimelightIO(String limelight) {
		...
		DoubleSubscriber pipelineLatencySub =
				table.getTable(limelight).getDoubleTopic("tl").subscribe(0.0);
		DoubleSubscriber captureLatencySub =
				table.getTable(limelight).getDoubleTopic("cl").subscribe(0.0);
